...
(:types agent place locatable - object
...
(:action Drive
:parameters (?a - agent ?x - truck ?y - place ?z - place) 
:precondition (and (located ?x ?y))
:effect (and (not (located ?x ?y)) (located ?x ?z)
		(increase (fuel-cost) 10)))

(:action Lift
:parameters (?a - agent ?x - hoist ?y - crate ?z - surface ?p - place)
...
(:action Load
:parameters (?a - agent ?x - hoist ?y - crate ?z - truck ?p - place)

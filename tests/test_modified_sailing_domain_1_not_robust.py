...
domain_str = """(define (domain sailing)
    ...
    (:action go-north-east
        :parameters (?b - agent)
	:precondition (and )
    :effect (and (increase (total-cost) 1.0) (increase (x ?b) 1.5) (increase (y ?b) 1.5))
	)
    (:action go-north-west
        :parameters (?b - agent)
	:precondition (and )
        :effect (and (increase (total-cost) 1.0) (decrease (x ?b) 1.5) (increase (y ?b) 1.5))
	)
    (:action go-est
        :parameters (?b - agent)
	:precondition (and )
        :effect (and (increase (total-cost) 1.0) (increase (x ?b) 3))
	)
    (:action go-west
        :parameters (?b - agent)
	:precondition (and )
        :effect (and (increase (total-cost) 1.0) (decrease (x ?b) 3))
	)
    (:action go-south-west
        :parameters(?b - agent)
	:precondition (and )
        :effect (and (increase (total-cost) 1.0) (increase (x ?b) 2) (decrease (y ?b) 2))
	)
    (:action go-south-east
        :parameters(?b - agent)
	:precondition (and )
        :effect (and (increase (total-cost) 1.0) (decrease (x ?b) 2) (decrease (y ?b) 2))
	)
    (:action go-south
        :parameters(?b - agent)
	:precondition (and )
        :effect (and (increase (total-cost) 1.0) (decrease (y ?b) 2))
	)
    (:action save-person
